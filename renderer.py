...
class Renderer:
    ...
    def render_tanks(self, player, enemies):
        ...
        for e in enemies:
            ...
            from tanks.tanks import BasicTank, FastTank, ArmorTank, BossTank, PowerTank

            # Use type() check instead of isinstance if inheritance is causing issues, 
            # though isinstance should work if classes are distinct.
            if isinstance(e, PowerTank):
                _draw_tank(self.screen, rect, (255, 150, 0), WHITE, e.direction)
            elif isinstance(e, ArmorTank) and hasattr(e, 'hit_color'):
                col = e.hit_color()
                flash = getattr(e, '_hit_flash', 0) > 0 and (self.tick % 4 < 2)
    ...
    def render_pause(self):
        ...
        # Title
        title = self.font_xl.render("PAUSED", True, HUD_ACCENT)
        self.screen.blit(title, title.get_rect(center=(cx, cy - 60)))

        # Options
        opt1 = self.font_med.render("ENTER TO CONTINUE", True, WHITE)
        self.screen.blit(opt1, opt1.get_rect(center=(cx, cy + 20)))

        opt2 = self.font_med.render("Q TO QUIT", True, (255, 80, 80))
        self.screen.blit(opt2, opt2.get_rect(center=(cx, cy + 60)))
    ...
    def render_menu(self):
        ...
        self.screen.blit(start, start.get_rect(center=(CX, my + 40)))

        # Level Selection Hint
        lvls = self.font_med.render("PRESS  1, 2, or 3  FOR LEVEL SELECT", True, WHITE)
        self.screen.blit(lvls, lvls.get_rect(center=(CX, my + 80)))

        # Controls
        controls = self.font_sm.render(
            "WASD / Arrows = Move    SPACE = Fire    ESC = Quit",
            True, MID_GRAY)
        self.screen.blit(controls, controls.get_rect(center=(CX, my + 115)))
